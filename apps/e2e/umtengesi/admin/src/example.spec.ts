...
/**
 * Smoke test for the umtengesi-admin remote, exercised through the shell host
 * the way a real user reaches it: navigate the shell to `/umtengesi-admin`,
 ...
 */
test('redirects to login when reaching the umtengesi-admin remote (auth required)', async ({
  page,
}) => {
  await page.goto('/umtengesi-admin');
