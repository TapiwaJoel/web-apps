...
/**
 * Login flow for the shell-admin host, against a stubbed gateway.
 *
 ...
 *
 * Note this shell has no `/dashboard` route (unlike shell-web / shell-client);
 * its guarded post-login landing page is `/app-selector`.
 */
...
test.describe('shell-admin login', () => {
  ...
  test('signs in and navigates to the returnUrl', async ({ page }) => {
    ...
    // Authenticated from here on, so requiredAuthGuard lets /app-selector through.
    await stubPermissions(page, true);

    await page.goto('/login?returnUrl=%2Fapp-selector');
