...
export class SubscriptionsService {
  ...
  public create(dto: CreateSubscriptionDto): Observable<SubscriptionResponseDto> {
    return this.http
      .post<ServiceResponse<SubscriptionResponseDto>>(this.url(), dto)
      .pipe(
        map((r: ServiceResponse<SubscriptionResponseDto>): SubscriptionResponseDto => r.data),
  ...
  public list(
    ...
    return this.http
      .get<ServiceResponse<PaginateResult<SubscriptionResponseDto>>>(this.url(), {
        params,
      })
      .pipe(
        map(
  ...
  private url(...segments: string[]): string {
    return buildUrl(this.baseUrl, LEARNING_MANAGEMENT_PATH, 'subscriptions', ...segments);
  }
}
